...
class Pokemon(Global):
    ...
    def button_back(self):
        ...
        # Ajuster les coordonnées pour rester dans le cadre du programme
        rect_x = min(10, screen_width - rect_width - 10)
        rect_y = 10

        # Afficher le rectangle
        self.rect_radius(10, self.white, rect_x, rect_y, rect_width, rect_height)

        # Afficher le texte "BACK" au centre du rectangle
        text_x = rect_x + (rect_width - self.police_c1.size("BACK")[0]) // 2
        text_y = rect_y + (rect_height - self.police_c1.size("BACK")[1]) // 2
        self.text_c1("BACK", self.black, text_x, text_y)

        # Vérifier si un bouton de la souris est enfoncé
        mouse_click = pygame.mouse.get_pressed()
        if mouse_click[0] == 1:  # Clic gauche
            # Ajoutez ici le code pour effectuer l'action "BACK"
